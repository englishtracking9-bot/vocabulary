...
- 記憶技巧（字根字首記憶法）：單字卡／先讀列表／字根分頁衍生字顯示「🧠 記憶聯想」（把字根意思串成一句）與「🔡 照音節拼」（僅當各部位剛好拼回單字才顯示，不亂拆）；同字根家族把「✓ 已學會」的字標成綠色錨點，從已知推未知。資料由 `scripts/tag_roots.py` 產生（root 家族 92＋prefix 安全擴充 250＝342 字有拆解／聯想，280 字有音節）。
